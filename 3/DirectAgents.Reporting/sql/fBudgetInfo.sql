...
SELECT ISNULL(BudgetInfo.MediaSpend, Campaign.MediaSpend) AS MediaSpend
	...
	, ISNULL(CASE WHEN PlatformBudgetInfo.Date <= @StartDate THEN PlatformBudgetInfo.Date END,
	  ...
	  ISNULL(@StartDate, '1/1/1970')))
	  AS AsOfDate
	, ISNULL(CASE WHEN BudgetInfo.Date <= @StartDate THEN BudgetInfo.Date END,
	  ISNULL(@StartDate, '1/1/1970'))
	  AS MediaSpendAsOfDate
	...
	, Account.PlatformId
	, 'C' + CAST(Campaign.Id AS varchar) + ISNULL('-BID' + CONVERT(varchar, BudgetInfo.Date, 112), '') + ISNULL('-A' + CAST(Account.Id AS varchar), '') + ISNULL('-P' + CAST(Platform.Id AS varchar), '') + ISNULL('-PBID' + CONVERT(varchar, PlatformBudgetInfo.Date, 112), '') AS UniqueId
FROM td.Campaign
