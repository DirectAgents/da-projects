DECLARE @SynchDate DATE = (SELECT MAX([date]) FROM [dbo].[carhartt_ca_non_branded_sp_sov_brand_sv])

DELETE FROM [dbo].[carhartt_ca_non_branded_sp_sov_brand_sv]
WHERE [date] >= @SynchDate;

WITH sp_non_branded_with_sv
AS
(
	SELECT
		[sph].[phrase],
...
(
    ...
    FROM sp_non_branded_with_sv
    WHERE [datetime] >= @SynchDate
),
sp_non_branded_with_rn_grouped
AS
(
    SELECT
        ...
        SUM(rn)					AS [rn_sum]
    FROM sp_non_branded_with_rn
    GROUP BY [datetime], [phrase]
),
sp_non_branded_with_wieghted_sv
AS
(
    SELECT
        [a].[datetime],
    ...
    FROM sp_non_branded_with_rn AS a
    LEFT JOIN sp_non_branded_with_rn_grouped b ON   [a].[datetime] = [b].[datetime]
...
DECLARE @SynchDate DATE = (SELECT MAX([date]) FROM [dbo].[carhartt_ca_non_branded_sp_sov_search_term_sv])

DELETE FROM [dbo].[carhartt_ca_non_branded_sp_sov_search_term_sv]
WHERE [date] >= @SynchDate;
