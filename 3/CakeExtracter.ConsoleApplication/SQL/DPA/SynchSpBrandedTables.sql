DECLARE @SynchDate DATE = (SELECT MAX([date]) FROM [dbo].[carhartt_paid_branded_sp_brand_sv])

DELETE FROM [dbo].[carhartt_paid_branded_sp_brand_sv]
...
(
    ...
    FROM [dbo].[carhartt_paid_branded_sp_hourly_scraperApi]
...
INSERT INTO carhartt_paid_branded_sp_brand_sv
SELECT
...
DECLARE @SynchDate DATE = (SELECT MAX([date]) FROM [dbo].[carhartt_paid_branded_sp_search_term_sv])

DELETE FROM [dbo].[carhartt_paid_branded_sp_search_term_sv]
WHERE [date] >= @SynchDate;
...
INSERT INTO carhartt_paid_branded_sp_search_term_sv
SELECT
    CAST([datetime] AS DATE)    AS [date],
    [phrase]                    AS [search_term],
