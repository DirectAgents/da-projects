...
namespace CakeExtracter.Etl.TradingDesk.LoadersDA
{
    ...
    /// </summary>
    class AdformCampaignSummaryLoader : Loader<StrategySummary>
    {
        ...
        public AdformCampaignSummaryLoader(int accountId = -1)
        {
            ...
            strategyRepository = new AdformStrategyRepository();
        }

        protected override int Load(List<StrategySummary> items)
        {
            Logger.Info(AccountId, "Loading {0} AdformStrategySummaries..", items.Count);
            PrepareData(items);
        ...
        public int UpsertDailySummaries(List<StrategySummary> items)
        {
            ...
            Logger.Info(AccountId, "Saving {0} AdformStrategySummaries ({1} updates, {2} additions, {3} duplicates, {4} deleted, {5} already-deleted, {6} skipped)",
        ...
        private void TryToAddSummary(ClientPortalProgContext db, StrategySummary item, IEnumerable<int> strategyIdsInDb, LoadingProgress progress)
        {
            ...
            {
                Logger.Warn(AccountId, "Skipping load of item. AdformStrategy with id {0} does not exist.", item.StrategyId);
        ...
        private void TryToUpdateSummary(ClientPortalProgContext db, StrategySummary item, StrategySummary target, LoadingProgress progress)
        {
            ...
            {
                Logger.Warn(AccountId, "Encountered duplicate for {0:d} - AdformStrategy {1}", item.Date, item.StrategyId);
        ...
        private void TryToAddStrategy(ClientPortalProgContext db, Strategy strategyProps)
        {
            ...
            Logger.Info(accountId, "Saved new AdformStrategy: {0} ({1}), ExternalId={2}", strategy.Name, strategy.Id, strategy.ExternalId);
        }
        ...
        private void TryToUpdateStrategy(ClientPortalProgContext db, Strategy strategy, Strategy strategyProps)
        {
            ...
            Logger.Info(accountId, "Updated AdformStrategy: {0}, Eid={1}", strategyProps.Name, strategyProps.ExternalId);
            if (numUpdates > 1)
            {
